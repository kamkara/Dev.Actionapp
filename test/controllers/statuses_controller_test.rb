...
class StatusesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create status" do
    assert_difference("Status.count") do
      post statuses_url, params: { status: { name: @status.name, slug: @status.slug, user_id: @status.user_id } }
    end
  ...
  test "should update status" do
    patch status_url(@status), params: { status: { name: @status.name, slug: @status.slug, user_id: @status.user_id } }
